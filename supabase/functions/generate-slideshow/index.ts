...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: profile } = await admin
      .from("profiles")
      .select("plan, default_image_source")
      .eq("id", userId)
      .single();
    ...
    const { data: capOk, error: capErr } = await admin.rpc("check_and_increment_ai_cost", {
      _user_id: userId,
      _cost_cents: 3,
    });
    ...
    if (capOk === false) {
      await admin
        .from("slideshows")
        .update({ status: "failed", generation_error: "Monthly AI cost cap reached for your plan." })
        .eq("id", slideshowId);
    ...
    // Read photo config from request body (explicit, sent by Generating.tsx)
    ...
    const photoMeta = photoLayout
      ? {
          photo_layout: photoLayout,
          image_source: photoOneSource,
          image_url: photoOneUrl,
          image_id: photoOneId,
          hook: photoHook,
          value: photoValue,
          cta: photoCta,
    ...
    const chosenStyle =
      (insights?.best_style as any) && !isExploration
        ? (insights.best_style as string)
    ...
    const userNonProduct = (allImages || [])
      .filter((i: any) => !i.is_product_shot)
    ...
    if (productShots.length === 0) {
      await admin
        ...
        .update({
          status: "failed",
          generation_error: 'No product shot in workspace. Upload at least one in "Product slide images".',
        })
        .eq("id", slideshowId);
    ...
    if (imageSource === "both") {
      const { data: stock } = await admin
        .from("stock_images")
        .select("id, ai_description, ai_tags, public_url, category");
    ...
    if (nonProduct.length === 0 && photoLayout === null) {
      await admin
        .from("slideshows")
        .update({ status: "failed", generation_error: "No images available. Upload images or enable stock images." })
        .eq("id", slideshowId);
    ...
    const { data: recentSs } = await admin
      .from("slideshows")
...
${
  photoLayout !== "one"
    ? `AVAILABLE IMAGES (pick ${needNonProduct} DISTINCT indexes â€” never repeat the same index, prefer images NOT marked [recently-used]):
...
- Match each image to the slide's actual content.`
    : "IMAGE: the user has pre-selected an image â€” image_index is irrelevant, use 0 for all slides."
}
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            cta_text: {
              type: "string",
              description: "Final CTA block, 1-3 short lowercase sentences separated by \\n. Last sentence is the CTA.",
            },
          },
...
Use these learnings to bias your decisions. Do not mention this context in the output.`;
    ...
    await admin
      ...
      .update({
        generation_progress: {
          ...
          step_index: 2,
          total_steps: 4,
          message: "Selecting and arranging images...",
          percent: 60,
        },
      })
      .eq("id", slideshowId);
    ...
    const resolveSlot = async (
      slot: any,
    ): Promise<{ image_id: string | null; image_url: string | null; is_stock: boolean }> => {
      if (!slot || slot.source === "ai") return { image_id: null, image_url: null, is_stock: false };
      if (slot.source === "stock" && slot.image_url)
        return { image_id: null, image_url: slot.image_url, is_stock: true };
    ...
    periodStart2.setDate(1);
    const ps = periodStart2.toISOString().slice(0, 10);
    const { data: existing } = await admin
      .from("usage")
      .select("id, slideshows_generated")
      .eq("user_id", userId)
      .eq("period_start", ps)
      .maybeSingle();
    if (existing) {
      await admin
        .from("usage")
        .update({ slideshows_generated: (existing.slideshows_generated || 0) + 1 })
        .eq("id", existing.id);
    } else {
